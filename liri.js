...
var spotify = new Spotify(liri.spotify);
...
if (inputArg === "my-tweets") {
    myTweets();
} else if (inputArg === "spotify-this-song") {
    spotifyThis();
} else if (inputArg === "movie-this") {
    pickMovie();
} else if (inputArg === "do-what-it-says") {
    pickRequest();
}
...
function spotifyThis() {
    if (requestName) {
        ...
        spotify.request('https://api.spotify.com/v1/search?type=track')
            .then(function (data) {
                console.log(data);
